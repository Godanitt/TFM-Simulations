...
parameter_data = pd.read_csv(
    os.path.join(DATA_DIR_PAR, "ArN2_IR_primary.csv")
)["parameter"].to_numpy()
...
for i, electric_field in enumerate(electric_fields):
    ...
    yield_teo = theory_yield_ArN2_Ir_696(parameter_data, subset, fN2, pressure)  / 100 * W_ArN2(fN2) / 0.2
    yield_teo += theory_yield_ArN2_Ir_727(parameter_data, subset, fN2, pressure) / 100 * W_ArN2(fN2) / 0.2
    yield_teo += theory_yield_ArN2_Ir_750(parameter_data, subset, fN2, pressure) / 100 * W_ArN2(fN2) / 0.2
    yield_teo += theory_yield_ArN2_Ir_763(parameter_data, subset, fN2, pressure) / 100 * W_ArN2(fN2) / 0.2
    yield_teo += theory_yield_ArN2_Ir_772(parameter_data, subset, fN2, pressure) / 100 * W_ArN2(fN2) / 0.2
