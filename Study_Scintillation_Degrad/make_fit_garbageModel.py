...
for name in name_SP_CF3[1:]:
    x0 = [1.0, 10/100]
    res = opt.least_squares(fun_to_min_vis, x0, args=(name,), bounds=([0, 0], [100, 10]))
    fit_vis.loc["autoQ", name] = res.x[0]
    fit_vis.loc["C", name]     = res.x[1]

for name in name_SP_CF4[1:]:
    x0 = [100.0, 10/100]
    res = opt.least_squares(fun_to_min_uv, x0, args=(name,), bounds=([0, 0], [1000,100]))
...
plt.tight_layout()
plt.savefig("output/vis_fit_data_garbageModel.pdf")
...
plt.legend(title="Curvas", fontsize=8)
plt.tight_layout()
plt.savefig("output/uv_fit_data_garbageModel.pdf")
